...
cpc1 as (
SELECT *,
ROW_NUMBER() OVER (PARTITION BY rdbms_id,vendor_code ORDER BY start_date asc) as seq
FROM cpcbookings
),

cpc2 as (
SELECT *,
ROW_NUMBER() OVER (PARTITION BY rdbms_id,vendor_code ORDER BY start_date asc)-1 as seq
FROM cpcbookings
),
