...
2) CHANGE FILTERING IN LINE 291 to 295
...
WITH prefinal as (
SELECT a.*
FROM pandata_ap_commercial.monthly_target_list_V2 a
...
SELECT a.*
FROM final a
left join `fulfillment-dwh-production.pandata_curated.sf_accounts` sf on a.global_entity_id = sf.global_entity_id and a.vendor_code = sf.vendor_code
left join `fulfillment-dwh-production.pandata_curated.gsheet_ncr_exclusion_cities_pk` ec on a.city_name = ec.exclusion_city -- excluding expansions city list. Check city names tally with pd_vendors: https://docs.google.com/spreadsheets/d/1heYgVTNgQZRxKVkFQ3GIx3fb6gp7dk2d4z5FvmpoOeM/edit#gid=0

where a.global_entity_id = 'FP_PK'
and sf.vertical_segment <> "Home Based Kitchen"
and sf.vendor_grade NOT IN ("AAA","A","B")
and a.gmv_class NOT IN ("A","B")
and ec.exclusion_city is null
