...
2) CHANGE FILTERING IN LINE 287 & 288
...
vendor_code,
account_owner,
is_reopened_in_backend
FROM `foodpanda-ph-bigquery.general.gsheet_commercial_bir_offlined_vendors`
where is_reopened_in_backend = FALSE)

SELECT a.*,
FROM (
SELECT a.*,
FROM final a
LEFT JOIN `foodpanda-ph-bigquery.general.gsheet_ssc_target_list` b
ON a.vendor_code = b.vendor_code
where a.global_entity_id = 'FP_PH'
and (a.owner_name = 'Incubation Program PH' OR b.vendor_code is not null)
) a
LEFT JOIN offline o
on a.vendor_code = o.vendor_code
where o.is_reopened_in_backend IS NULL OR o.is_reopened_in_backend IS TRUE
