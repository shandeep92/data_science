...
and date(bk.created_at_utc) >= DATE_SUB(DATE_TRUNC(CURRENT_DATE(),MONTH),INTERVAL 3 MONTH)
...
user,
channel,
case when channel <> "Local" then channel else concat(channel," ",vendor_grade) end as final_source,
...
global_entity_id,
vendor_code,
format_date('%Y%V',created_date_local) as yearweek,
...
where CASE WHEN current_date() = DATE_TRUNC(current_date(),WEEK(MONDAY)) THEN year_week < FORMAT_DATE('%Y%V',DATE_TRUNC(CURRENT_DATE(),WEEK(MONDAY))) 
ELSE year_week <= FORMAT_DATE('%Y%V',DATE_TRUNC(CURRENT_DATE(),WEEK(MONDAY))) END
