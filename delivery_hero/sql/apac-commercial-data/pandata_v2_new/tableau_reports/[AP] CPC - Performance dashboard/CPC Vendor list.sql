...
LEFT JOIN `fulfillment-dwh-production.pandata_curated.pps_vendors_promo_areas` vpa ON pa.global_entity_id = vpa.global_entity_id AND pa.uuid = vpa.pps_promo_area_uuid
LEFT JOIN `fulfillment-dwh-production.pandata_curated.pps_vendors` v ON v.global_entity_id = vpa.global_entity_id AND v.uuid = vpa.pps_vendor_uuid
...
select *,
IFNULL(case when status = 'cancelled' and end_date < last_day then final_spent_clicks else budgeted_clicks end,0) as final_budgeted_clicks
...
where yearmonth_live >= FORMAT_DATE('%Y%m',DATE_SUB(CURRENT_DATE(),INTERVAL 4 MONTH))
)))
...
final_budget_eur,
budgeted_clicks,
final_budgeted_clicks,
final_spent_clicks,
cpc_rev_local,
cpc_rev_eur,
case when cpc_orders is null then 0 else cpc_orders end as cpc_orders,
utilization,
MAX(yearmonth_live) OVER (PARTITION BY global_entity_id,vendor_code) as max_yearmonth,
MAX(end_date) OVER (PARTITION BY global_entity_id,vendor_code) as max_end_date,
CASE 
WHEN status = 'cancelled' THEN MAX(end_date) OVER (PARTITION BY global_entity_id,vendor_code) 
END AS churn_date,
FORMAT_DATE('%b-%Y',CASE 
WHEN status = 'cancelled' THEN MAX(end_date) OVER (PARTITION BY global_entity_id,vendor_code) 
END) AS churn_month_year,
FORMAT_DATE('%Y%m',CASE 
WHEN status = 'cancelled' THEN MAX(end_date) OVER (PARTITION BY global_entity_id,vendor_code) 
END) AS churn_yearmonth
FROM final_cpc

ORDER BY global_entity_id,vendor_code,yearmonth_live asc)

where end_date = max_end_date
and yearmonth_live = max_yearmonth),
...
SELECT 
f.*,
IFNULL(SAFE_DIVIDE(promo_areas_booked,promo_areas_available),0) as perc_promo_areas_booked,
o.avg_open_hrs_per_day,
c.churn_date,
c.churn_month_year,
c.churn_yearmonth,
case when churn_date is null then "No" else "Yes" end as churned,
case when (f.cpc_roi <=1 OR f.cpc_roi IS NULL) then "<=1"
when f.cpc_roi >1 and f.cpc_roi <2 then "1-2"
when f.cpc_roi >=2 and f.cpc_roi <3 then "2-3"
when f.cpc_roi >=3 and f.cpc_roi <4 then "3-4"
when f.cpc_roi >=4 and f.cpc_roi <5 then "4-5"
when f.cpc_roi >=5 and f.cpc_roi <10 then "5-10"
when f.cpc_roi >= 10 then ">=10"
end as roi_banding


FROM final_cpc f
LEFT JOIN churned c on f.global_entity_id = c.global_entity_id and f.vendor_code = c.vendor_code
LEFT JOIN open_hours o on f.global_entity_id = o.global_entity_id and f.vendor_code = o.vendor_code and f.yearmonth_live = o.year_month



ORDER BY global_entity_id,vendor_code,yearmonth_live,start_date asc
