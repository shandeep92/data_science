...
""";
-- master products table for MENA
CREATE OR REPLACE TABLE `{{ params.project_id }}.cl_data_science_qcommerce._ccc_master_products_mena`
AS
...
WITH enrich AS (
    ...
    -- Filtering for MENA
	WHERE products.country_code IN (SELECT DISTINCT country_code FROM `fulfillment-dwh-production.cl_dmart.sources` WHERE region = 'MENA')
), unnested AS (
